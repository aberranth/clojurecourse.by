...
(defn data-table [tbl]
  (map (partial data-record (table-keys tbl)) (rest tbl)))
...
(defn where* [data condition-func]
  (if (nil? condition-func) data (filter condition-func data)))
...
(select student)
;; => [{:id 1, :year 1998, :surname "Ivanov"}
;;     {:id 2, :year 1997, :surname "Petrov"}
;;     {:id 3, :year 1996, :surname "Sidorov"}]
